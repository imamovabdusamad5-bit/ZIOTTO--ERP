            ...
            <div className="w-full h-full flex flex-col overflow-hidden relative bg-white">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
                    <div className="flex items-center gap-4">
                        <button onClick={onClose} className="p-2 bg-slate-200 hover:bg-red-100 hover:text-red-500 rounded-xl transition-colors text-slate-500">
                            <X size={20} />
                        </button>
                        <div>
                            <h2 className="text-2xl font-black text-slate-800 tracking-wide uppercase">{model?.name || 'Yangi Model'}</h2>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Artikul: {model?.code || 'N/A'}</p>
                        </div>
                    </div>
                ...
                {/* Tabs */}
                <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-3 z-10 relative shadow-sm">
                    {[
                        { id: 'kartasi', label: 'Model Kartasi', icon: Box, activeClass: 'bg-indigo-50 text-indigo-600 border-indigo-200 shadow-sm ring-1 ring-indigo-500/20 scale-[1.02]' },
                        { id: 'xarajatlar', label: 'Xarajatlar Varaqasi', icon: CircleDollarSign, activeClass: 'bg-emerald-50 text-emerald-600 border-emerald-200 shadow-sm ring-1 ring-emerald-500/20 scale-[1.02]' },
                        { id: 'ish_rejasi', label: 'Ish Rejasi (Jadval)', icon: Scissors, activeClass: 'bg-rose-50 text-rose-600 border-rose-200 shadow-sm ring-1 ring-rose-500/20 scale-[1.02]' },
                        { id: 'xronologiya', label: 'Xronologiya', icon: History, activeClass: 'bg-amber-50 text-amber-600 border-amber-200 shadow-sm ring-1 ring-amber-500/20 scale-[1.02]' }
                    ].map(tab => (
                        <button
                            key={tab.id}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                            ...
                            <div className="lg:col-span-4 flex flex-col gap-6">
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col gap-5">
                                    <h3 className="text-slate-800 font-black text-xs uppercase tracking-widest border-b border-slate-100 pb-3">Model Haqida Ma'lumot</h3>
                                    
                                    <div className="h-64 w-full p-4 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center relative group">
                                        {model?.image_url ? (
                                        ...
                                        <div className="absolute inset-0 bg-white/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <span className="text-slate-800 text-xs font-bold bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-200 backdrop-blur-md">Kattalashtirish</span>
                                        </div>
                                    </div>
                                    
                                    <div className="flex flex-col gap-1">
                                        <p className="text-lg font-bold text-slate-800">{model?.name}</p>
                                        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Yaratilgan: {new Date(model?.created_at).toLocaleDateString()}</p>
                                    </div>
                                    
                                    <button className="py-3 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700 rounded-xl text-xs font-black uppercase tracking-widest transition-all border border-indigo-200 active:scale-95">
                                        Ishlab Chiqarishga Tasdiqlash
                                    </button>
                                    <button className="py-3 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 rounded-xl text-xs font-black uppercase tracking-widest transition-all border border-red-200 active:scale-95">
                                        Shablonlardan O'chirish
                                    </button>
                                    
                                    <div className="mt-2">
                                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 block">Izoh (Eslatmalar)</label>
                                        <textarea 
                                            readOnly 
                                            value={model?.notes?.map(n => n.text).join('\n') || 'Izoh yo\'q'} 
                                            className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-700 min-h-[100px] resize-none focus:outline-none"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-4 flex flex-col gap-6">
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col gap-5 h-full">
                                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                        <h3 className="text-slate-800 font-black text-xs uppercase tracking-widest">Ranglar Palitrasi</h3>
                                        <button onClick={addColor} className="flex items-center gap-1 text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest transition-colors bg-blue-50 px-2 py-1 rounded-md border border-blue-100">
                                    ...
                                    <div className="flex flex-col gap-4 overflow-y-auto pr-1 custom-scrollbar">
                                        {colors.length === 0 ? (
                                            <div className="text-center py-10 text-slate-400 text-xs font-bold uppercase tracking-widest border border-dashed border-slate-200 rounded-xl">Ranglar Kiritilmagan</div>
                                        ) : (
                                            colors.map((c, i) => (
                                                <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col gap-3 relative group transition-all hover:border-slate-300 shadow-sm">
                                                    <button onClick={() => removeColor(i)} className="absolute top-3 right-3 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                                                        <Trash2 size={14} />
                                                    </button>
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full border border-slate-300 flex-shrink-0 shadow-sm" style={{backgroundColor: c.name || '#f1f5f9'}}></div>
                                                        <div className="flex-1">
                                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Rang Nomi</label>
                                                            <input 
                                                                type="text" 
                                                                value={c.name} 
                                                                onChange={e => updateColor(i, 'name', e.target.value)} 
                                                                placeholder="Masalan: Qora, Oq, Qormelanj"
                                                                className="w-full bg-transparent border-none text-sm font-bold text-slate-800 placeholder-slate-400 focus:outline-none"
                                                            />
                                                        </div>
                                                    </div>
                                                    <div className="border-t border-slate-200 pt-3 mt-1">
                                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1 block">TNVED Kodi</label>
                                                        <input 
                                                            type="text" 
                                                            value={c.tnved} 
                                                            onChange={e => updateColor(i, 'tnved', e.target.value)} 
                                                            placeholder="TNVED (Ixtiyoriy)"
                                                            className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-700 placeholder-slate-400 focus:outline-none focus:border-blue-400 transition-colors shadow-sm"
                                                        />
                                                    </div>
                                                </div>
                                            ))
                            ...
                            <div className="lg:col-span-4 flex flex-col gap-6">
                                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex flex-col gap-6 h-full">
                                    
                                    {/* SIZES */}
                                    <div className="flex-1 bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col shadow-sm">
                                        <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-3">
                                            <h3 className="text-slate-800 font-black text-sm uppercase tracking-widest">1. Razmer</h3>
                                            <button onClick={() => addStringItem(setSizes, sizes)} className="text-xs font-black text-emerald-600 hover:text-emerald-700 uppercase tracking-widest bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 transition-colors flex items-center gap-1">
                                        ...
                                        <div className="flex flex-wrap gap-2 overflow-y-auto custom-scrollbar pr-1 max-h-[120px]">
                                            {sizes.length === 0 && <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">Kiritilmagan</span>}
                                            {sizes.map((s, i) => (
                                                <div key={i} className="flex items-center bg-white border border-slate-200 shadow-sm rounded-lg overflow-hidden group">
                                                    <input 
                                                        type="text" 
                                                        ...
                                                        className="bg-transparent text-xs font-bold text-slate-800 px-3 py-2 w-32 focus:outline-none focus:bg-slate-50 transition-colors"
                                                    />
                                                    <button onClick={() => removeStringItem(setSizes, sizes, i)} className="px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors border-l border-slate-100">
                                                        <X size={12} />
                                                    </button>
                                                </div>
                                            ))}
                                    ...
                                    <div className="flex-1 bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col shadow-sm">
                                        <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-3">
                                            <h3 className="text-slate-800 font-black text-sm uppercase tracking-widest">2. Komplektatsiya</h3>
                                            <button onClick={() => addStringItem(setComponents, components)} className="text-xs font-black text-purple-600 hover:text-purple-700 uppercase tracking-widest bg-purple-50 px-3 py-1.5 rounded-lg border border-purple-100 transition-colors flex items-center gap-1">
                                                <Plus size={14} /> Qo'shish
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 overflow-y-auto custom-scrollbar pr-1 max-h-[120px]">
                                            {components.length === 0 && <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">Kiritilmagan</span>}
                                            {components.map((c, i) => (
                                                <div key={i} className="flex items-center bg-white border border-slate-200 shadow-sm rounded-lg overflow-hidden group">
                                                    <input 
                                                        type="text" 
                                                        ...
                                                        className="bg-transparent text-xs font-bold text-slate-800 px-3 py-2 w-32 focus:outline-none focus:bg-slate-50 transition-colors"
                                                    />
                                                    <button onClick={() => removeStringItem(setComponents, components, i)} className="px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors border-l border-slate-100">
                                                        <X size={12} />
                                                    </button>
                                                </div>
                                            ))}
                                    ...
                                    <div className="flex-1 bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col shadow-sm">
                                        <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-3">
                                            <h3 className="text-slate-800 font-black text-sm uppercase tracking-widest">3. Mavsum</h3>
                                            <button onClick={() => addStringItem(setSeasons, seasons)} className="text-xs font-black text-amber-600 hover:text-amber-700 uppercase tracking-widest bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-100 transition-colors flex items-center gap-1">
                                                <Plus size={14} /> Qo'shish
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 overflow-y-auto custom-scrollbar pr-1 max-h-[120px]">
                                            {seasons.length === 0 && <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">Kiritilmagan</span>}
                                            {seasons.map((s, i) => (
                                                <div key={i} className="flex items-center bg-white border border-slate-200 shadow-sm rounded-lg overflow-hidden group">
                                                    <input 
                                                        type="text" 
                                                        ...
                                                        className="bg-transparent text-xs font-bold text-slate-800 px-3 py-2 w-32 focus:outline-none focus:bg-slate-50 transition-colors"
                                                    />
                                                    <button onClick={() => removeStringItem(setSeasons, seasons, i)} className="px-2 py-2 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors border-l border-slate-100">
                                                        <X size={12} />
                                                    </button>
                                                </div>
                                            ))}
