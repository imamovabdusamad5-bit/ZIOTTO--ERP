        ...
        <div className="bg-[#161b22] rounded-[3rem] shadow-2xl border border-white/5 overflow-hidden">
            ...
            <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left">
                    <thead>
                        <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50/5">
                        ...
                        {logs.map(log => (
                            <tr key={log.id} className="hover:bg-white/5 transition-colors">
                                ...
                                <td className="px-8 py-6">
                                    ...
                                    <div className="text-[9px] text-gray-500 font-bold uppercase">{log.inventory?.category}</div>
                                </td>
                                <td className="px-8 py-6">
                                    <span className="px-2 py-1 bg-white/5 text-gray-400 rounded-lg text-[10px] font-black uppercase border border-white/10">{log.inventory?.color || '---'}</span>
                                </td>
                                ...
                                <td className="px-8 py-6">
                                    <div className="text-xs font-medium text-gray-500 max-w-xs">{log.reason || '---'}</div>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="md:hidden divide-y divide-white/5">
                {logs.map(log => (
                    <div key={log.id} className="p-4 space-y-3 bg-[#161b22]">
                        <div className="flex justify-between items-start">
                            <div>
                                <div className="font-black text-white uppercase text-sm">{log.inventory?.item_name || '---'}</div>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="text-[9px] font-bold text-indigo-400 uppercase px-1.5 py-0.5 bg-indigo-500/10 rounded">{log.inventory?.color}</span>
                                </div>
                            </div>
                            <div className="text-right">
                                ...
                                <div className={`mt-1 font-black text-sm ${log.type === 'In' ? 'text-emerald-500' : 'text-rose-500'}`}>
                                    {log.type === 'In' ? '+' : '-'}{log.quantity}
                                </div>
                            </div>
                        </div>
                        <div className="flex justify-between items-center text-[10px]">
                            <span className="text-gray-500 font-medium">{new Date(log.created_at).toLocaleString('uz-UZ', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' })}</span>
                            <span className="text-gray-400 italic font-medium">{log.reason}</span>
                        </div>
                    </div>
                ))}
            </div>
