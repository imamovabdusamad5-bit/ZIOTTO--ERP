        ...
        <div className="space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-3xl font-black text-white tracking-tight">Rejalashtirish (Excel)</h2>
                        <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-[10px] mt-1">Model detallari, ranglar va gramaj bo'yicha aniq hisob-kitob</p>
                    </div>
                </div>
                {!showForm && (
                    <button
            ...
            {showForm && (
                <div className="bg-[#161b22] rounded-[3rem] shadow-4xl border border-white/5 overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
                    <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-white">Yangi Reja Formasi</h3>
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Barcha detallarni aniqlik bilan kiriting</p>
                            </div>
                        </div>
                        <button onClick={() => setShowForm(false)} className="p-4 bg-white/5 text-gray-500 hover:text-white rounded-2xl transition-all">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    <form onSubmit={handleSave} className="p-10 space-y-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Model Tanlash</label>
                                <select
                                    required
                                    className="w-full px-6 py-4 bg-black/40 border border-white/10 rounded-2xl font-bold text-white outline-none focus:border-emerald-500 transition-all appearance-none"
                                    value={selectedModel?.id || ''}
                                    onChange={e => handleModelChange(e.target.value)}
                                >
                                    <option value="">Tanlang...</option>
                                    {models.map(m => <option key={m.id} value={m.id} className="bg-[#161b22] text-white">{m.name}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Artikul (Kod)</label>
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Buyurtma №</label>
                                <input
                                    className="w-full px-6 py-4 bg-black/40 border border-white/10 rounded-2xl font-bold text-white outline-none focus:border-emerald-500 transition-all placeholder:text-gray-700"
                                    placeholder="ORD-001"
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Topshirish Muddat</label>
                                <input
                                    className="w-full px-6 py-4 bg-black/40 border border-white/10 rounded-2xl font-bold text-white outline-none focus:border-emerald-500 transition-all"
                                    type="date"
                        ...
                        {selectedModel && (
                            <>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="overflow-x-auto border border-white/5 rounded-[2rem] shadow-4xl bg-black/20 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
                                        <table id="planning-table" className="w-full min-w-max text-left border-separate border-spacing-0">
                                            <thead>
                                                {/* TOP LEVEL GROUP HEADERS */}
                                                <tr className="text-white text-[11px] uppercase font-black tracking-widest text-center shadow-lg">
                                                    {/* Fixed Left Groups */}
                                                    ...
                                                    <th className="py-4 border-b border-white/10 bg-[#161b22]"></th>
                                                </tr>

                                                {/* COLUMN HEADERS */}
                                                <tr className="bg-[#0d1117] text-gray-200 text-[10px] uppercase font-bold tracking-wider text-center">
                                                    {/* Fixed Left Columns */}
                                                    <th className="sticky left-0 bg-[#0d1117] px-2 py-4 border-b border-r border-white/10 w-20 z-20 text-white shadow-[2px_0_5px_rgba(0,0,0,0.5)]">YOSHI/ RAZMER</th>
                                                    <th className="sticky left-[80px] bg-[#0d1117] px-2 py-4 border-b border-r border-white/10 w-16 z-20 text-white shadow-[2px_0_5px_rgba(0,0,0,0.5)]">ASSORT</th>
                                                    <th className="sticky left-[144px] bg-[#0d1117] px-2 py-4 border-b border-r border-white/10 w-20 z-20 text-white shadow-[2px_0_5px_rgba(0,0,0,0.5)]">SONI</th>

                                                    {/* 1. Colors Group */}
                                                    {/* Main Color is always first */}
                                                    <th className="px-2 py-4 border-b border-r border-white/10 text-pink-200 min-w-[120px] bg-pink-900/20">ASOSIY RANG</th>
                                                    {uniqueParts.map(part => (
                                                        <th key={`col-${part}`} className="px-2 py-4 border-b border-r border-white/10 text-pink-200 bg-pink-900/20 min-w-[120px]">{part.toUpperCase()} RANGI</th>
                                                    ))}

                                                    {/* 2. Gramaj Group */}
                                                    {uniqueParts.map(part => (
                                                        <th key={`gram-${part}`} className="px-2 py-4 border-b border-r border-white/10 text-slate-200 bg-slate-800/40 w-28">GRAMAJ {part.toUpperCase()}</th>
                                                    ))}

                                                    {/* 3. Total Group */}
                                                    {uniqueParts.map(part => (
                                                        <th key={`tot-${part}`} className="px-2 py-4 border-b border-r border-white/10 text-amber-200 bg-amber-900/20 w-28">KG {part.toUpperCase()}</th>
                                                    ))}

                                                    <th className="px-2 py-4 border-b border-white/10 w-10"></th>
                                                </tr>
                                            </thead>
                                                ...
                                                {breakdown.map((row, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={row.id} className="hover:bg-white/[0.02] transition-colors group">
                                                            ...
                                                            <td className={`sticky left-0 bg-[#0d1117] border-r border-white/10 group-hover:bg-[#161b22] px-2 py-1 z-10 align-top`}>
                                                                <input
                                                                    className={`w-full px-1 py-2 bg-transparent border-none rounded-lg text-xs font-black text-center outline-none transition-all ${isSameSizeAsPrev ? 'text-gray-500 focus:text-white' : 'text-white'}`}
                                                            ...
                                                            {/* 2. ASSORT */}
                                                            <td className="sticky left-[80px] bg-[#0d1117] group-hover:bg-[#161b22] px-2 py-1 border-r border-white/10 z-10">
                                                                <input
                                                                    type="number"
                                                                    className="w-full px-1 py-2 bg-transparent text-white rounded-lg text-xs font-bold text-center outline-none focus:bg-white/10 placeholder:text-gray-700"
                                                                    value={row.assort || ''}
                                                            ...
                                                            {/* 3. SONI */}
                                                            <td className="sticky left-[144px] bg-[#0d1117] group-hover:bg-[#161b22] px-2 py-1 border-r border-white/10 z-10">
                                                                <input
                                                                    type="number"
                                                            ...
                                                            {/* --- GROUP 1: COLORS --- */}
                                                            {/* Main Color */}
                                                            <td className="px-2 py-1 border-r border-white/10 bg-pink-900/10">
                                                                <input
                                                            ...
                                                            {uniqueParts.map(part => (
                                                                <td key={`c-${part}`} className="px-2 py-1 border-r border-white/10 bg-pink-900/10">
                                                                    <input
                                                            ...
                                                            {/* --- GROUP 2: GRAMAJ --- */}
                                                            {uniqueParts.map(part => (
                                                                <td key={`g-${part}`} className="px-2 py-1 border-r border-white/10 bg-slate-800/20">
                                                                    <input
                                                                        type="number"
                                                                        step="0.001"
                                                                        className="w-full px-1 py-2 bg-transparent text-slate-200 text-center font-mono text-[11px] outline-none hover:bg-white/5 rounded-lg focus:bg-slate-500/30 transition-all placeholder:text-gray-600 border border-transparent focus:border-slate-500/30"
                                                            ...
                                                            {uniqueParts.map(part => {
                                                                ...
                                                                return (
                                                                    <td key={`t-${part}`} className="px-2 py-1 border-r border-white/10 bg-amber-900/10 text-center">
                                                                        <span className={`font-mono text-[11px] font-black ${parseFloat(total) > 0 ? 'text-amber-200' : 'text-gray-700'}`}>
                                    ...
                                    <button
                                        type="button"
                                        onClick={addRow}
                                        className="w-full py-4 border border-dashed border-white/10 rounded-[2rem] text-gray-500 font-black text-xs uppercase tracking-widest hover:bg-white/5 hover:border-emerald-500/30 hover:text-emerald-500 transition-all flex items-center justify-center gap-3 group"
                                    >
                                ...
                                <div className="space-y-6 pt-10 border-t border-white/5">
                                    <div className="flex items-center gap-4 px-2">
                                        ...
                                        <div>
                                            <h4 className="text-lg font-black text-white uppercase tracking-tight">Ranglar bo'yicha Ehtiyoj</h4>
                                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">Xom-ashyo sarfi hisob-kitobi</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {Object.entries(summary).map(([color, items]) => (
                                            <div key={color} className="bg-white/[0.03] rounded-[2.5rem] p-8 border border-white/5 hover:border-indigo-500/30 transition-all flex flex-col gap-6 group">
                                                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <span className="font-black text-white uppercase tracking-tighter text-lg">{color}</span>
                                                    </div>
                                                </div>
                                                <div className="space-y-4">
                                                    {items.map((item, i) => (
                                                        <div key={i} className="flex flex-col gap-1.5 p-3 rounded-2xl hover:bg-white/5 transition-all">
                                                            ...
                                                            <div className="flex items-center justify-between text-[8px] font-bold uppercase tracking-widest">
                                                                ...
                                                                <span className="text-gray-600">Zaxira: {item.waste}%</span>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                ...
                                <div className="flex justify-end pt-10 border-t border-white/5 gap-4">
                                    <button
                                        ...
                                        className="px-10 py-5 bg-white/5 text-gray-400 rounded-3xl font-black text-xs uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all border border-white/5"
                                    >
                                        Bekor Qilish
                                    </button>
                                    <button
            ...
            {showSummaryModal && (
                <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
                    <div id="printable-area" className="bg-[#161b22] rounded-[3.5rem] shadow-4xl border border-white/10 w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300">
                        <div className="p-10 border-b border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black text-white tracking-tight">Umumiy Sotib Olish (Plan)</h3>
                                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">{selectedOrders.length} ta model bo'yicha yig'indi</p>
                                </div>
                            </div>
                            <button onClick={() => setShowSummaryModal(false)} className="w-12 h-12 flex items-center justify-center rounded-full hover:bg-white/5 text-gray-500 hover:text-white transition-all no-print">
                                <X size={24} />
                            </button>
                        </div>
                        <div className="p-10 overflow-y-auto custom-scrollbar space-y-10">
                            {Object.keys(grandTotal).length === 0 ? (
                                <div className="text-center py-20 text-gray-400 font-bold">Ma'lumot topilmadi</div>
                            ) : (
                                <>
                                    {/* MATOLAR Section */}
                                    <div className="space-y-6">
                                        <h4 className="flex items-center gap-3 text-sm font-black text-white uppercase tracking-[0.2em] border-l-4 border-emerald-500 pl-4">
                                            MATOLAR (Fabrics)
                                        </h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {Object.values(grandTotal).filter(i => i.type === 'Mato').map((item, idx) => (
                                                <div key={idx} className="bg-white/[0.03] border border-white/5 p-6 rounded-[2rem] flex justify-between items-center group hover:border-emerald-500/30 transition-all">
                                                    <div>
                                                        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">{item.color}</p>
                                                        <h5 className="font-black text-white text-lg leading-tight uppercase">{item.name}</h5>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-2xl font-black text-white">{item.val.toFixed(2)}</p>
                                                        <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest">{item.unit}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        <h4 className="flex items-center gap-3 text-sm font-black text-white uppercase tracking-[0.2em] border-l-4 border-amber-500 pl-4">
                                            AKSESSUARLAR (Accessories)
                                        </h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                            {Object.values(grandTotal).filter(i => i.type !== 'Mato').map((item, idx) => (
                                                <div key={idx} className="bg-white/[0.03] border border-white/5 p-5 rounded-3xl flex justify-between items-center group hover:border-amber-500/30 transition-all">
                                                    <div>
                                                        <h5 className="font-bold text-gray-300 text-xs leading-tight mb-1">{item.name}</h5>
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-[8px] text-gray-600 font-black uppercase">{item.unit}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                        ...
                        <div className="p-10 border-t border-white/5 bg-white/[0.02] flex justify-end gap-4 no-print">
                            <button onClick={() => window.print()} className="px-10 py-5 bg-white/5 text-white font-black text-xs uppercase tracking-widest rounded-2xl border border-white/10 hover:bg-white/10 flex items-center gap-3 transition-all">
                                <FileText size={20} />
                                Chop etish / PDF
                            </button>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-white tracking-tight uppercase">Faol Rejalar</h3>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5">Hozirda jarayonda bo'lgan buyurtmalar</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {orders.map(order => (
                        <div key={order.id} className={`bg-[#161b22] border rounded-[2.5rem] p-8 shadow-2xl transition-all relative overflow-hidden group ${selectedOrders.includes(order.id) ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-white/5 hover:border-emerald-500/30'}`}>
                            ...
                            <div className="flex items-start justify-between mb-8">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <h4 className="text-2xl font-black text-white tracking-tight">{order.order_number}</h4>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <input
                                        type="checkbox"
                                        className="w-8 h-8 rounded-xl border-white/10 bg-white/5 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-8 pt-6 border-t border-white/5">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-black text-white truncate">{order.models?.name}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Jami Soni</p>
                                    <p className="text-sm font-black text-white">{order.total_quantity} <span className="text-emerald-500/50 text-[10px] tracking-widest font-bold">DONA</span></p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Muddat</p>
                                ...
                                <div className="space-y-1">
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Holat</p>
                                ...
                                <div className="pt-6 border-t border-white/5">
                                    ...
                                    <div className="space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
                                        {Object.entries(order.material_summary).map(([color, items]) => (
                                            <div key={color} className="bg-white/5 border border-white/5 p-4 rounded-2xl hover:border-amber-500/30 transition-all">
                                                    ...
                                                    {items.map((item, idx) => (
                                                        <div key={idx} className="flex justify-between items-center text-xs">
                                                            <div className="flex-1">
                                                                ...
                                                                {item.type && (
                                                                    <span className="ml-2 text-[8px] text-gray-600 uppercase">({item.type})</span>
                                                                )}
                                                            </div>
                                                            <div className="text-right">
